...
文档职责：定义 LABOR-SAVING Product Entity 的基础层，明确产品实体、生命周期、与行业、案例、文章、视频、下载和 FAQ 的关联关系。M2.4 Product System Foundation 已通过验收并冻结为 v1.0；产品首次发布验收作为跨阶段质量门禁保留，后台 CMS、CRM 和数据库实现由 M3 承接。
...
CMS、Database、CRM 和批量导出由 M3 Website Platform Foundation 承接，历史 M2.6-M2.8 编号通过 `docs/MILESTONE_MAPPING.md` 保留映射。

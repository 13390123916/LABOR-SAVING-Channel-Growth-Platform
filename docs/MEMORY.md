...
- 2026-07-21 完成 M3.4 Lead Center Architecture 架构冻结：新增 `docs/LEAD_CENTER.md` 与 ADR-0011。确认 `docs/PLATFORM_ARCHITECTURE.md` 是整个 Platform 的结构入口，阅读顺序为 README -> Platform Architecture -> Roadmap -> ADR；ADR 只记录重大平台方向决策，不为字段微调、上传策略、导出格式或列表筛选单独创建。Lead Domain 覆盖 Lead Entity、Lead Type、字段分层、来源归因、生命周期、分配、去重、隐私、导出边界、CRM 输入、权限和审计；M3.5-M3.9 继续做架构冻结，Prisma、Migration、Database、RBAC、CMS、Media、Lead、SEO Runtime、API 和 Admin UI 统一进入 M4 Platform Runtime。
